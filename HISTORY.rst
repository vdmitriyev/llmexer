...
* **Breaking:** ``experiment generate`` now writes two tables per provider — ``experiment_<provider>`` (identity, prompt, results, hashes) and ``params_<provider>`` (one row per hyperparameter set, keyed by ``(params_code, profile_name)`` where ``params_code`` is ``<model_name>_<provider>``) — so parameter values are stored once per profile instead of repeated on every row. Databases generated by earlier versions are rejected on open; re-run ``experiment generate``.
