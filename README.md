...
| `fix` | Rewrites the stored answers, one named fix per run. `--ensure-json-format` repairs an answer that holds JSON wrapped in a fence or followed by an explanation, so the whole cell parses as JSON again. `--rename-json-field OLD NEW` renames a top-level field of that JSON, for a prompt whose wording changed after part of the experiment had run. A database needing both takes two runs, repair first. Every change is logged in the `datafix_logs` table with the old value, the new value and a note saying which fix it was. Nothing is written unless you pass `--apply`; without it, `--test N` sets how many proposed changes are printed for you to check. Supports `--file` and `--dry-run`. | `llmexer experiment fix --pid my-project --ensure-json-format --apply` |
