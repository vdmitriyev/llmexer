...
Every try is appended to `try_experiment_<provider>` and `try_param_<provider>` in the experiment database (`--file`, newest by default), so a series of tries is kept as a history and each one records the parameters it actually ran with. The generated experiment rows, their results and `experiment stats` are untouched. Option `--dry-run` prints the rendered prompt without calling anything.
