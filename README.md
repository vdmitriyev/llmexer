...
| `fix` | Repairs answers that hold JSON wrapped in a fence or followed by an explanation, so the whole cell parses as JSON again. Every change is logged in the `datafix_logs` table with the old and the new value. Nothing is written unless you pass `--apply`; without it, `--test N` sets how many proposed changes are printed for you to check. Supports `--file` and `--dry-run`. | `llmexer experiment fix --pid my-project --apply` |
