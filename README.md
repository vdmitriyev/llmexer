...
| `add-agreement` | Adds `analyse_experiment_agreements.ipynb`, which reads the flattened CSV the experiment notebook writes and reports Cohen's kappa for every pair of provider, model and profile, one row per yes/no field. Name the fields with `--fields`, or leave `--autodetect-fields` on to have the notebook find them. The modules it calls are copied alongside, so it also works without `init`. Existing files are only replaced with `--rewrite`, which backs them up first. Supports `--dry-run`. | `llmexer analysis add-agreement --pid my-project` |
